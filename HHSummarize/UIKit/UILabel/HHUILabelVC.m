...
//    [lable sizeToFit]; //让label自适应
    ...
    HHCopyLabel *label1 = [[HHCopyLabel alloc] initWithFrame:CGRectMake(10, 300, 200, 40)];
...
//    muParagraph.paragraphSpacing = 20;        // 段距
    ...
    NSAttributedString *attrText = [[NSAttributedString alloc] initWithString:@"长按可以跳转" attributes:@{ NSParagraphStyleAttributeName: muParagraph }];

    label1.attributedText = [self rechargeMobileAttributedString];

    [self.view addSubview:self.htmlLab];
}
- (NSAttributedString *)rechargeMobileAttributedString {
    NSMutableAttributedString *mAttributed = [[NSMutableAttributedString alloc] init];
    
    NSMutableAttributedString *mPre = [[NSMutableAttributedString alloc] initWithString:@"充值手机号 "];
    NSRange preRange = NSMakeRange(0, mPre.length);
    [mPre addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:16] range:preRange];
    [mPre addAttribute:NSForegroundColorAttributeName value:[UIColor greenColor] range:preRange];
    [mAttributed appendAttributedString:mPre];
    
    NSMutableAttributedString *mMobile = [[NSMutableAttributedString alloc] initWithString:@"一百万元一百万元一百万元一百万元一百万元一百万元"];
    NSRange mobileRange = NSMakeRange(0, mMobile.length);
    [mMobile addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:18] range:mobileRange];
    [mMobile addAttribute:NSForegroundColorAttributeName value:[UIColor yellowColor] range:mobileRange];
    
    
    NSMutableParagraphStyle *muParagraph = [[NSMutableParagraphStyle alloc]init];
    muParagraph.lineSpacing = 10;        // 行距
    //    muParagraph.paragraphSpacing = 20;        // 段距
    muParagraph.firstLineHeadIndent = 50;        // 首行缩进
    
    
    
    NSAttributedString *attrText = [[NSAttributedString alloc] initWithString:mMobile.string
                                                                   attributes:@{ NSParagraphStyleAttributeName: muParagraph,NSFontAttributeName:[UIFont systemFontOfSize:18],NSForegroundColorAttributeName:[UIColor yellowColor] }];
    [mAttributed appendAttributedString:attrText];
