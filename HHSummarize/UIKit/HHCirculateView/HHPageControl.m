...
@interface HHPageControl ()<HHPageControlDelegate>
...
-(void)setType:(HHPageControlType)type{
    if (_type != type) {
        _type = type;
        [self createPointView];
    }
...
-(void)createPointView{
    ...
    if(self.frame.size.width<mainWidth){
        ...
        if (_type == HHPageControlLeft) {
            startX = 10;
        }else if (_type == HHPageControlMiddle){
            startX = (self.frame.size.width-mainWidth)/2.0;
        }else if (_type == HHPageControlRight){
